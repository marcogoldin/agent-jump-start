...
test("root AGENTS.md/AGENT.md, CLAUDE.md, and copilot instructions mirrors are generated from .agents/AGENTS.md", () => {
  const tempDir = makeTempDir();
  try {
    const specPath = writeSpec(tempDir, makeMinimalSpec());
    expectSuccess(runCli(["render", "--spec", specPath, "--target", tempDir]));
...
test("sync does not warn about manifest-managed generated skill packages", () => {
  const tempDir = makeTempDir();
  try {
    const specPath = writeSpec(tempDir, makeMinimalSpec({
    ...
    assert.doesNotMatch(secondSync.stdout, /invalid local skill package/i);
    assert.doesNotMatch(secondSync.stdout, /managed-skill: This SKILL\.md was generated by Agent Jump Start/i);
  } finally {
    cleanupTempDir(tempDir);
  }
});
...
test("external skill intake end-to-end imports local skills and sync propagates them across targets", () => {
  ...
  try {
    ...
    // After intake, the operator-authored skill file becomes managed on sync.
    // --force acknowledges the conscious opt-in: sync rewrites it with the
    // rendered version (which now carries the provenance marker).
...
test("sync repairs deleted and modified outputs in a single run", () => {
  ...
  try {
    ...
    // Simulate realistic drift: operator hand-edited a managed file but left
    // the Agent Jump Start provenance header in place. Sync must still heal it.
    writeFileSync(
      join(tempDir, ".agents/AGENTS.md"),
      "# Workspace Instructions\n\n> Generated by Agent Jump Start v0.0.0 from spec.yaml.\n\n# manual drift\n",
      "utf8",
    );
...
// ===========================================================================
// P0: Preserve pre-existing agent files on first write
// ===========================================================================
...
test("classifyExistingFile distinguishes absent, managed, unmanaged, and not-a-file", () => {
  const tempDir = makeTempDir();
  try {
    const abs = (p) => join(tempDir, p);
    assert.equal(classifyExistingFile(abs("missing.md")), "absent");

    writeFileSync(abs("managed.md"), "> Generated by Agent Jump Start v1.2.3\nhello\n", "utf8");
    assert.equal(classifyExistingFile(abs("managed.md")), "managed");

    writeFileSync(abs("hand-written.md"), "# my own notes\n", "utf8");
    assert.equal(classifyExistingFile(abs("hand-written.md")), "unmanaged");

    mkdirSync(abs("CLAUDE.md"));
    assert.equal(classifyExistingFile(abs("CLAUDE.md")), "not-a-file");
  } finally {
    cleanupTempDir(tempDir);
  }
});

test("classifyPreWriteCollisions buckets paths by ownership", () => {
  const tempDir = makeTempDir();
  try {
    writeFileSync(join(tempDir, "managed.md"), "> Generated by Agent Jump Start v0.0.0\n", "utf8");
    writeFileSync(join(tempDir, "hand.md"), "# hand\n", "utf8");
    const result = classifyPreWriteCollisions({
      "managed.md": "new content",
      "hand.md": "new content",
      "new.md": "new content",
    }, tempDir);
    assert.deepEqual(result.managed, ["managed.md"]);
    assert.deepEqual(result.unmanaged, ["hand.md"]);
    assert.deepEqual(result.absent, ["new.md"]);
  } finally {
    cleanupTempDir(tempDir);
  }
});
...
test("applyWriteDecisions refuses to write when an unmanaged path has no decision", () => {
  ...
  try {
    writeFileSync(join(tempDir, "hand.md"), "# hand\n", "utf8");
    assert.throws(
      () => applyWriteDecisions({ "hand.md": "new" }, tempDir),
      /Refusing to overwrite unmanaged pre-existing file/,
    );
    assert.equal(readFileSync(join(tempDir, "hand.md"), "utf8"), "# hand\n");
  } finally {
    cleanupTempDir(tempDir);
  }
});

test("applyWriteDecisions keep skips the write and backup-then-overwrite creates a sibling backup", () => {
  const tempDir = makeTempDir();
  try {
    writeFileSync(join(tempDir, "keep.md"), "# kept\n", "utf8");
    writeFileSync(join(tempDir, "backup.md"), "# original\n", "utf8");
    const result = applyWriteDecisions(
      { "keep.md": "new-keep", "backup.md": "new-backup" },
      tempDir,
      { decisions: { "keep.md": "keep", "backup.md": "backup-then-overwrite" }, stamp: "fixed-stamp" },
    );
    assert.deepEqual(result.kept, ["keep.md"]);
    assert.equal(readFileSync(join(tempDir, "keep.md"), "utf8"), "# kept\n");
...
test("resolveConflictPolicyFromOptions rejects mutually exclusive flags", () => {
  assert.equal(resolveConflictPolicyFromOptions({}), null);
  assert.equal(resolveConflictPolicyFromOptions({ force: true }), "force");
  assert.equal(resolveConflictPolicyFromOptions({ backup: true }), "backup");
  assert.equal(resolveConflictPolicyFromOptions({ "keep-existing": true }), "keep");
  assert.throws(
    () => resolveConflictPolicyFromOptions({ force: true, backup: true }),
    /Conflicting flags/,
  );
});

function seedUnmanagedAgentFiles(tempDir) {
  mkdirSync(join(tempDir, ".github/instructions"), { recursive: true });
  ...
  mkdirSync(join(tempDir, ".windsurf/rules"), { recursive: true });
  mkdirSync(join(tempDir, ".clinerules"), { recursive: true });
  mkdirSync(join(tempDir, ".roo/rules"), { recursive: true });
  mkdirSync(join(tempDir, ".continue/rules"), { recursive: true });
  writeFileSync(join(tempDir, "CLAUDE.md"), "SENTINEL-CLAUDE\n", "utf8");
  writeFileSync(join(tempDir, "AGENTS.md"), "SENTINEL-AGENTS\n", "utf8");
  writeFileSync(join(tempDir, "AGENT.md"), "SENTINEL-AGENT-SINGULAR\n", "utf8");
  writeFileSync(join(tempDir, "GEMINI.md"), "SENTINEL-GEMINI\n", "utf8");
  writeFileSync(join(tempDir, ".github/copilot-instructions.md"), "SENTINEL-COPILOT\n", "utf8");
  writeFileSync(join(tempDir, ".github/instructions/general.instructions.md"), "SENTINEL-COPILOT-PATHSPEC\n", "utf8");
  writeFileSync(join(tempDir, ".cursor/rules/agent-instructions.mdc"), "SENTINEL-CURSOR\n", "utf8");
  writeFileSync(join(tempDir, ".amazonq/rules/general.md"), "SENTINEL-AMAZONQ\n", "utf8");
  writeFileSync(join(tempDir, ".junie/AGENTS.md"), "SENTINEL-JUNIE-AGENTS\n", "utf8");
  writeFileSync(join(tempDir, ".junie/guidelines.md"), "SENTINEL-JUNIE-GUIDELINES\n", "utf8");
  writeFileSync(join(tempDir, ".windsurf/rules/general.md"), "SENTINEL-WINDSURF-RULES\n", "utf8");
  writeFileSync(join(tempDir, ".windsurfrules"), "SENTINEL-WINDSURF\n", "utf8");
  writeFileSync(join(tempDir, ".clinerules/general.md"), "SENTINEL-CLINE\n", "utf8");
  writeFileSync(join(tempDir, ".roo/rules/agent-instructions.md"), "SENTINEL-ROO\n", "utf8");
  writeFileSync(join(tempDir, ".roorules"), "SENTINEL-ROO-LEGACY\n", "utf8");
  writeFileSync(join(tempDir, ".continue/rules/agent-instructions.md"), "SENTINEL-CONTINUE\n", "utf8");
  writeFileSync(join(tempDir, "CONVENTIONS.md"), "SENTINEL-AIDER\n", "utf8");
}

function assertSentinelsIntact(tempDir) {
  assert.match(readFileSync(join(tempDir, "CLAUDE.md"), "utf8"), /SENTINEL-CLAUDE/);
  assert.match(readFileSync(join(tempDir, "AGENTS.md"), "utf8"), /SENTINEL-AGENTS/);
  assert.match(readFileSync(join(tempDir, "AGENT.md"), "utf8"), /SENTINEL-AGENT-SINGULAR/);
  assert.match(readFileSync(join(tempDir, "GEMINI.md"), "utf8"), /SENTINEL-GEMINI/);
  assert.match(readFileSync(join(tempDir, ".github/copilot-instructions.md"), "utf8"), /SENTINEL-COPILOT/);
  assert.match(readFileSync(join(tempDir, ".github/instructions/general.instructions.md"), "utf8"), /SENTINEL-COPILOT-PATHSPEC/);
  assert.match(readFileSync(join(tempDir, ".cursor/rules/agent-instructions.mdc"), "utf8"), /SENTINEL-CURSOR/);
  assert.match(readFileSync(join(tempDir, ".amazonq/rules/general.md"), "utf8"), /SENTINEL-AMAZONQ/);
  assert.match(readFileSync(join(tempDir, ".junie/AGENTS.md"), "utf8"), /SENTINEL-JUNIE-AGENTS/);
  assert.match(readFileSync(join(tempDir, ".junie/guidelines.md"), "utf8"), /SENTINEL-JUNIE-GUIDELINES/);
  assert.match(readFileSync(join(tempDir, ".windsurf/rules/general.md"), "utf8"), /SENTINEL-WINDSURF-RULES/);
  assert.match(readFileSync(join(tempDir, ".windsurfrules"), "utf8"), /SENTINEL-WINDSURF/);
  assert.match(readFileSync(join(tempDir, ".clinerules/general.md"), "utf8"), /SENTINEL-CLINE/);
  assert.match(readFileSync(join(tempDir, ".roo/rules/agent-instructions.md"), "utf8"), /SENTINEL-ROO/);
  assert.match(readFileSync(join(tempDir, ".roorules"), "utf8"), /SENTINEL-ROO-LEGACY/);
  assert.match(readFileSync(join(tempDir, ".continue/rules/agent-instructions.md"), "utf8"), /SENTINEL-CONTINUE/);
...
test("sync refuses to overwrite unmanaged pre-existing agent files in non-interactive mode", () => {
  ...
  try {
    ...
    const result = runCli(["sync", "--spec", specPath, "--target", tempDir]);
    expectFailure(result);
    assert.match(result.stdout + result.stderr, /does not carry the Agent Jump Start provenance marker|Refused to overwrite pre-existing files/);
...
test("render refuses to overwrite unmanaged pre-existing agent files in non-interactive mode", () => {
  const tempDir = makeTempDir();
  try {
    seedUnmanagedAgentFiles(tempDir);
    const specPath = writeSpec(tempDir, makeMinimalSpec());

    const result = runCli(["render", "--spec", specPath, "--target", tempDir]);
    expectFailure(result);
    assertSentinelsIntact(tempDir);
  } finally {
    cleanupTempDir(tempDir);
...
test("sync --force overwrites unmanaged files when the operator opts in", () => {
  const tempDir = makeTempDir();
  try {
    seedUnmanagedAgentFiles(tempDir);
    const specPath = writeSpec(tempDir, makeMinimalSpec());

    const result = runCli(["sync", "--spec", specPath, "--target", tempDir, "--force"]);
    expectSuccess(result);
    assert.match(result.stdout, /Overwrote pre-existing operator-authored files/);
    assert.doesNotMatch(readFileSync(join(tempDir, "CLAUDE.md"), "utf8"), /SENTINEL-CLAUDE/);
    assert.match(readFileSync(join(tempDir, "CLAUDE.md"), "utf8"), /Agent Jump Start/);
  } finally {
    cleanupTempDir(tempDir);
  }
});

test("sync --backup creates timestamped backups before overwriting unmanaged files", () => {
  const tempDir = makeTempDir();
  try {
    seedUnmanagedAgentFiles(tempDir);
    const specPath = writeSpec(tempDir, makeMinimalSpec());

    const result = runCli(["sync", "--spec", specPath, "--target", tempDir, "--backup"]);
    expectSuccess(result);
    assert.match(result.stdout, /Backed up then overwrote pre-existing files/);

    const claudeBackups = readdirSync(tempDir).filter((n) => n.startsWith("CLAUDE.md.ajs-backup-"));
    assert.equal(claudeBackups.length, 1, "a single CLAUDE.md backup must be created");
    assert.match(readFileSync(join(tempDir, claudeBackups[0]), "utf8"), /SENTINEL-CLAUDE/);
    assert.doesNotMatch(readFileSync(join(tempDir, "CLAUDE.md"), "utf8"), /SENTINEL-CLAUDE/);
  } finally {
    cleanupTempDir(tempDir);
  }
...
test("sync conflicting flags fail loudly", () => {
  const tempDir = makeTempDir();
  try {
    const specPath = writeSpec(tempDir, makeMinimalSpec());
    const result = runCli(["sync", "--spec", specPath, "--target", tempDir, "--force", "--backup"]);
    expectFailure(result);
...
test("sync continues to rewrite managed (marker-bearing) files without prompting", () => {
  const tempDir = makeTempDir();
  try {
    const specPath = writeSpec(tempDir, makeMinimalSpec());
    expectSuccess(runCli(["sync", "--spec", specPath, "--target", tempDir]));

    // Operator edits a managed file but keeps the provenance header in place.
    const claudePath = join(tempDir, "CLAUDE.md");
    const drifted = readFileSync(claudePath, "utf8") + "\n<!-- operator drift -->\n";
    writeFileSync(claudePath, drifted, "utf8");

    const result = runCli(["sync", "--spec", specPath, "--target", tempDir]);
    expectSuccess(result);
    assert.doesNotMatch(readFileSync(claudePath, "utf8"), /operator drift/);
  } finally {
    cleanupTempDir(tempDir);
  }
});

test("manifest cleanStaleFiles never touches unmanaged adjacent files", () => {
  const tempDir = makeTempDir();
  try {
    const specPath = writeSpec(tempDir, makeMinimalSpec({
      skills: [makeSkillFixture({ slug: "cleanup-test" })],
    }));
    expectSuccess(runCli(["sync", "--spec", specPath, "--target", tempDir]));

    // Drop an unmanaged neighbor inside the managed skill tree.
    const neighborPath = join(tempDir, ".agents/skills/cleanup-test/NOTES.md");
    writeFileSync(neighborPath, "SENTINEL-NEIGHBOR\n", "utf8");

    // Remove the skill from the spec and re-sync. The managed files should be
    // cleaned but the neighbor must survive.
    writeSpec(tempDir, makeMinimalSpec({ skills: [] }));
    const result = runCli(["sync", "--spec", specPath, "--target", tempDir]);
    expectSuccess(result);
