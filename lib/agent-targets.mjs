...
export const AGENT_IDS = [
  ...
  "junie", "openai-codex", "cursor",
  "windsurf", "cline", "roo-code", "continue-dev", "aider",
];

export const AGENT_ID_ALIASES = new Map([
  ["github-agents", "openai-codex"],
]);

export function normalizeAgentId(agentId) {
  return AGENT_ID_ALIASES.get(agentId) ?? agentId;
}

export function isKnownAgentId(agentId) {
  if (typeof agentId !== "string" || agentId.length === 0) return false;
  return AGENT_IDS.includes(normalizeAgentId(agentId));
}

export function formatValidAgentIds() {
  const aliases = [...AGENT_ID_ALIASES.entries()].map(([alias, canonical]) => `${alias} -> ${canonical}`);
  return `${AGENT_IDS.join(", ")}; deprecated aliases: ${aliases.join(", ")}`;
}

export const AGENT_DISPLAY_CATALOG = [
  { id: "claude-code", name: "Claude Code", files: "CLAUDE.md, .claude/skills/*/SKILL.md" },
  { id: "github-copilot", name: "GitHub Copilot", files: ".github/copilot-instructions.md, .github/instructions/*.instructions.md" },
  ...
  { id: "openai-codex", name: "OpenAI Codex", files: "AGENTS.md" },
  { id: "cursor", name: "Cursor", files: ".cursor/rules/*.mdc, .agents/skills/*/SKILL.md" },
...
// Cleanup targets for disabled agents.
// Entries ending with "/" are directories; others are files.
// Only exclusively-owned paths are listed — shared roots (e.g. `.github/`)
// are NOT included because disabling one agent must not remove directories
// that other agents or tools may also use.
// Legacy file targets (.windsurfrules, .roorules, .clinerules) are included
// so narrowing produces a truly clean repo.
export const AGENT_REGISTRY = new Map([
  ...
  ["github-copilot", {
    ...
    agentRoots: [],  // .github/ is shared with copilot-instructions, actions, etc.
  }],
  ["gemini-cli", {
    displayName: "Gemini CLI",
    renderPaths: ["GEMINI.md"],
    skillPaths: [],
    agentRoots: ["GEMINI.md"],
  }],
  ["amazon-q", {
  ...
  ["openai-codex", {
    ...
    skillPaths: [],
    agentRoots: [],  // .agents/ is canonical infrastructure, always rendered
  }],
  ["cursor", {
    displayName: "Cursor",
...
export const AGENT_DISCOVERY_RULES = [
  { mode: "exact", path: "AGENTS.md", toolOfOrigin: "codex", scope: "repo-wide", tier: 1 },
  { mode: "exact", path: "AGENTS.override.md", toolOfOrigin: "codex", scope: "repo-wide", tier: 1 },
  { mode: "exact", path: "AGENT.md", toolOfOrigin: "codex-legacy", scope: "legacy-flat", tier: 2 },
  {
    mode: "tree",
    root: ".",
    filenames: ["AGENTS.md", "AGENTS.override.md"],
    ignoreDirs: [".agents", ".git", "node_modules", "dist", "build", "coverage", ".next", ".turbo", ".cache"],
    toolOfOrigin: "codex",
    scope: "nested",
    tier: 1,
  },
  { mode: "exact", path: "CLAUDE.md", toolOfOrigin: "claude", scope: "repo-wide", tier: 1 },
  { mode: "exact", path: ".claude/CLAUDE.md", toolOfOrigin: "claude", scope: "project", tier: 1 },
  { mode: "exact", path: ".claude/settings.json", toolOfOrigin: "claude", scope: "project", tier: 1 },
  {
    mode: "tree",
    root: ".claude/skills",
    suffixes: ["SKILL.md"],
