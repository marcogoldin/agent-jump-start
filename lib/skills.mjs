...
// ---------------------------------------------------------------------------
// Agent Jump Start provenance detection
//
// A SKILL.md is considered "generated" when it carries one of the structured
// provenance signatures that `lib/renderers.mjs` emits. Re-importing such
// artifacts is a silent data-corruption path (P0-0 in the roadmap): the
// canonical categories collapse into synthetic buckets `Rule Categories by
// Priority` / `Quick Reference` / `Detailed Guidance` and rule ids recurse
// (`rcbp-*`, `qr-*`, `dg-*`).
//
// The detector accepts either the parsed frontmatter (preferred, authoritative
// signal) or the raw SKILL.md body (fallback for operator-scrubbed frontmatter
// or legacy artifacts that only carry the body blockquote).
// ---------------------------------------------------------------------------

const AJS_FRONTMATTER_VERSION = /^Agent Jump Start v(\d[\w.-]*)/;
const AJS_BODY_PROVENANCE = /^>\s*Generated by Agent Jump Start v(\d[\w.-]*)(?:\s+from\s+`([^`]+)`)?/m;

/**
 * Detect whether a SKILL.md was produced by Agent Jump Start's renderer.
 *
 * @param {string}      content     — full SKILL.md text (frontmatter + body)
 * @param {object|null} frontmatter — pre-parsed frontmatter, if available
 * @returns {null | { version: string, generatedFrom: string|null, source: "frontmatter"|"body" }}
 */
export function detectAgentJumpStartProvenance(content, frontmatter = null) {
  if (frontmatter && typeof frontmatter === "object") {
    const metadata = frontmatter.metadata;
    if (metadata && typeof metadata === "object" && typeof metadata.generated_by === "string") {
      const match = metadata.generated_by.match(AJS_FRONTMATTER_VERSION);
      if (match) {
        const generatedFrom = typeof metadata.generated_from === "string" && metadata.generated_from.trim()
          ? metadata.generated_from.trim()
          : null;
        return { version: match[1], generatedFrom, source: "frontmatter" };
      }
    }
  }

  if (typeof content === "string") {
    const match = content.match(AJS_BODY_PROVENANCE);
    if (match) {
      const generatedFrom = match[2] ? match[2].trim() : null;
      return { version: match[1], generatedFrom, source: "body" };
    }
  }
...
export function readSkillMdFile(filePath, skillDir = null) {
  ...
  // P0-0 guard (defense in depth): block the reader itself from parsing an
  // Agent Jump Start generated artifact. importSkillsIntoSpec carries a
  // richer diagnostic via assertNotGeneratedArtifact, but code paths that
  // bypass that boundary (e.g. update-skills → readExternalSkill) must also
  // refuse, otherwise a re-imported rendered skill would silently corrupt
  // the canonical spec with synthetic `rcbp-*` / `qr-*` / `dg-*` ids.
  ...
  if (provenance) {
    ...
    throw new Error(
      `${filePath}: refusing to read an Agent Jump Start generated SKILL.md ` +
      `(v${provenance.version}${fromHint}). Import the raw upstream skill instead.`,
    );
  }
