...
function findSkillCandidates(rootDir, maxDepth = 4, currentDepth = 0, matches = []) {
  ...
  for (const entry of entries) {
    ...
    findSkillCandidates(join(rootDir, entry.name), maxDepth, currentDepth + 1, matches);
  }
...
export function resolveSkillImportSource(source, options = {}) {
  ...
  if (provider === "github") {
    ...
    const importPath = treePath
      ? resolve(join(cloneDir, treePath))
      : resolveSkillPathCandidates(cloneDir, options.skill ?? null);
