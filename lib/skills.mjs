...
export function parseSkillMdFrontmatter(content) {
  ...
  const frontmatter = normalizeSkillFrontmatter(parseMinimalYaml(yamlBlock));

  return { frontmatter, body };
}

// Fields that are semantically single-line per the SKILL.md convention.
// Authors often use folded (`>`) or literal (`|`) block scalars for readability;
// we collapse any whitespace (including newlines) into single spaces so
// downstream consumers always receive a clean one-line string.
const SINGLE_LINE_STRING_FIELDS = new Set([
  "name",
  "description",
  "license",
  "author",
...
const SINGLE_LINE_METADATA_FIELDS = new Set([
  "title",
  "version",
  "author",
  "license",
...
function normalizeSkillFrontmatter(frontmatter) {
  if (!frontmatter || typeof frontmatter !== "object") return frontmatter;

  for (const field of SINGLE_LINE_STRING_FIELDS) {
    if (typeof frontmatter[field] === "string") {
      frontmatter[field] = collapseSingleLine(frontmatter[field]);
    }
  }

  if (frontmatter.metadata && typeof frontmatter.metadata === "object" && !Array.isArray(frontmatter.metadata)) {
    for (const field of SINGLE_LINE_METADATA_FIELDS) {
      if (typeof frontmatter.metadata[field] === "string") {
        frontmatter.metadata[field] = collapseSingleLine(frontmatter.metadata[field]);
      }
    }
  }
 ...
 * Does not handle: anchors, aliases, multi-document, flow sequences.
 */
function parseMinimalYaml(yaml) {
  ...
  // Recognise block scalar headers: `|`, `>`, plus optional chomping and
  // explicit indentation indicators (in either order), plus an optional
  // trailing YAML comment. Examples: `|`, `>-`, `|2-`, `> # note`, `|- #note`.
  // Returns { style, chomping, explicitIndent } or null.
  function parseBlockScalarHeader(marker) {
    // Strip an optional YAML inline comment (` #…` to end of line).
    const commentMatch = marker.match(/^(.*?)(?:\s+#.*)?$/);
    const stripped = (commentMatch ? commentMatch[1] : marker).trim();

    if (stripped.length === 0) return null;
    const style = stripped[0];
    ...
    for (let i = 1; i < stripped.length; i++) {
      ...
      if ((ch === "+" || ch === "-") && chomping === "") {
        chomping = ch;
      } else if (ch >= "1" && ch <= "9" && explicitIndent === 0) {
        explicitIndent = Number(ch);
      } else {
        return null;
      }
    }
    return { style, chomping, explicitIndent };
  }

  // Read a block scalar body starting at the current `index`.
  // parentIndent is the indentation of the owning key.
  function readBlockScalar(parentIndent, style, chomping, explicitIndent) {
    // Collect raw lines together with their "more-indented" flag so the
    // folding pass can apply the YAML 1.2 rule: more-indented lines
    // preserve line breaks around them instead of folding into spaces.
    const rawLines = [];
    let baseIndent = null;
    ...
    while (index < lines.length) {
      ...
      if (line.trim() === "") {
        rawLines.push({ content: "", empty: true, moreIndented: false });
        index++;
        continue;
      }
      ...
      if (baseIndent === null) {
        baseIndent = explicitIndent > 0 ? parentIndent + explicitIndent : lineIndent;
      }
      if (lineIndent < baseIndent) break;

      rawLines.push({
        content: line.slice(baseIndent),
        empty: false,
        moreIndented: lineIndent > baseIndent,
      });
      index++;
    }

    // Trailing empty lines participate in chomping, not in the body.
    let trailingEmpty = 0;
    while (rawLines.length > 0 && rawLines[rawLines.length - 1].empty) {
      rawLines.pop();
      trailingEmpty++;
    }
    ...
    let body;
    if (style === "|") {
      body = rawLines.map((entry) => entry.content).join("\n");
    } else {
      // Folded: adjacent regular non-empty lines join with a single space.
      // Empty lines become explicit newlines between paragraphs. Lines more
      // indented than the base indent are kept verbatim with line breaks
      // preserved before and after them, matching YAML 1.2 semantics.
      let body2 = "";
      let prev = "start"; // "start" | "empty" | "folded" | "moreIndented"
      for (const entry of rawLines) {
        ...
        if (entry.moreIndented) {
          if (prev === "folded" || prev === "moreIndented") {
            body2 += "\n";
          }
          body2 += entry.content;
          prev = "moreIndented";
    ...
    if (chomping === "+") {
      return body + "\n".repeat(trailingEmpty + (body.length > 0 ? 1 : 0));
    }
    // Clip (default): single trailing newline when non-empty.
  ...
  function parseBlock(minIndent) {
    ...
    while (index < lines.length) {
      ...
      const blockScalar = parseBlockScalarHeader(afterColon);
      if (blockScalar) {
        index++;
        obj[key] = readBlockScalar(indent, blockScalar.style, blockScalar.chomping, blockScalar.explicitIndent);
      } else if (afterColon === "") {
...
export function skillMdToAjsSkill(frontmatter, body, references = [], scripts = [], assets = []) {
  ...
  const slug = normalizeSkillSlug(frontmatter.name);
  ...
  const displayTitle = title ?? frontmatter.metadata?.title ?? frontmatter.name ?? slug;
