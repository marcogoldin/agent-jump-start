 ...
 */
export function inferSections(evidence) {
  const sections = [];
  ...
  if (hasTypeScript) {
    sections.push({
      ...
      rules: [
        { value: "Enable strict mode in tsconfig.json and do not weaken it with skipLibCheck or loose compiler flags.", provenance: "inferred", source: "tsconfig.json" },
        { value: "Avoid `any` type annotations. Use `unknown` with narrowing when the type is genuinely dynamic.", provenance: "inferred", source: "tsconfig.json" },
        { value: "Prefer explicit return types on exported functions and public API boundaries.", provenance: "inferred", source: "tsconfig.json" },
      ],
    });
  }
