...
import { detectCanonicalAgentIds, buildAgentSupportSpec } from "./agent-support.mjs";
import { AGENT_IDS, AGENT_DISPLAY_CATALOG, AGENT_REGISTRY, isKnownAgentId, normalizeAgentId } from "./agent-targets.mjs";
...
function choiceSearchTokens(choice) {
  return [choice.key, choice.label, ...(choice.aliases ?? [])]
    .filter(Boolean)
    .map((token) => token.toLowerCase());
}

function hasChoicePrefix(buffer, choiceDefinitions) {
  return choiceDefinitions.some((choice) =>
    choiceSearchTokens(choice).some((token) => token.startsWith(buffer)));
}

export function supportsRawModeChoice(input = process.stdin, env = process.env) {
  return !env.NO_COLOR && typeof input.setRawMode === "function";
}

function renderRawModeChoiceFrame(output, question, choiceDefinitions, selectedIndex, typedBuffer) {
  output.write("\x1B[?25l");
  output.write("\x1B[2K\x1B[G");
  output.write(`  ? ${question}\n`);
  choiceDefinitions.forEach((choice, index) => {
    output.write("\x1B[2K\x1B[G");
    output.write(
      index === selectedIndex
  ...
  output.write(
    typedBuffer
      ? `  typed: ${typedBuffer}\n`
      : "  use ↑/↓, j/k, or type a choice then press Enter\n",
  );
}

function finishRawModeChoice(input, output, resume, onKeypress) {
...
export function chooseWithRawMode({
  question,
  choiceDefinitions,
  defaultChoice,
  input = process.stdin,
  ...
  return new Promise((resolve, reject) => {
    ...
    let selectedIndex = Math.max(0, choiceDefinitions.findIndex((choice) => choice.key === defaultChoice));
    let typedBuffer = "";
    const frameLines = choiceDefinitions.length + 2;

    const rerender = () => {
      output.write(`\x1B[${frameLines}F`);
      renderRawModeChoiceFrame(output, question, choiceDefinitions, selectedIndex, typedBuffer);
    };

    const resolveChoice = (choiceKey) => {
      const selected = choiceDefinitions.find((choice) => choice.key === choiceKey);
      output.write("\x1B[J");
      output.write(`  \x1B[32m✔\x1B[0m ${question} · \x1B[36m${selected?.label ?? choiceKey}\x1B[0m\n`);
      finishRawModeChoice(input, output, resume, onKeypress);
      resolve(choiceKey);
    };
    ...
    const onKeypress = (str, key = {}) => {
      if (key.ctrl && key.name === "c") {
        finishRawModeChoice(input, output, resume, onKeypress);
        exitFn(1);
        reject(new Error("Interactive selection aborted by SIGINT."));
        return;
      }

      const canUseVimUp = typedBuffer === "" && !hasChoicePrefix("k", choiceDefinitions);
      const canUseVimDown = typedBuffer === "" && !hasChoicePrefix("j", choiceDefinitions);

      if (key.name === "up" || (key.name === "k" && canUseVimUp)) {
        selectedIndex = (selectedIndex - 1 + choiceDefinitions.length) % choiceDefinitions.length;
        rerender();
        return;
      }
      if (key.name === "down" || (key.name === "j" && canUseVimDown)) {
        selectedIndex = (selectedIndex + 1) % choiceDefinitions.length;
        rerender();
        return;
      }
      ...
      if (key.name === "return" || key.name === "enter") {
        if (typedBuffer) {
          const typedChoice = resolveChoiceInput(typedBuffer, choiceDefinitions, null);
          if (typedChoice) {
            resolveChoice(typedChoice);
            return;
          }
          typedBuffer = "";
          rerender();
        ...
        resolveChoice(choiceDefinitions[selectedIndex].key);
        return;
      }
      ...
      typedBuffer += str.toLowerCase();
      const matchedChoice = resolveChoiceInput(typedBuffer, choiceDefinitions, null);
      if (matchedChoice) {
        selectedIndex = Math.max(0, choiceDefinitions.findIndex((choice) => choice.key === matchedChoice));
      }
      rerender();
    };
...
// ---------------------------------------------------------------------------
// Multi-select raw-mode helper (zero-dependency checklist prompt)
// ---------------------------------------------------------------------------

const CHECKLIST_FOOTER_DEFAULT = "Use ↑/↓, Space to toggle, Enter to confirm (a=all, n=none)";

function formatChecklistRow(item, box) {
  const label = String(item.label ?? "");
  const sublabel = String(item.sublabel ?? "");
  const detail = String(item.detail ?? "");
  const parts = [box, label.padEnd(20), sublabel.padEnd(18)];
  if (detail) parts.push(detail);
  return parts.join(" ").trimEnd();
}

function renderChecklistFrame(output, question, items, selected, focusIndex, footerMessage) {
  output.write("\x1B[?25l");
  output.write("\x1B[2K\x1B[G");
  output.write(`  ? ${question}\n`);
  items.forEach((item, index) => {
    output.write("\x1B[2K\x1B[G");
    const box = selected.has(item.key) ? "[x]" : "[ ]";
    const line = formatChecklistRow(item, box);
    if (index === focusIndex) {
      output.write(`    \x1B[36m> ${line}\x1B[0m\n`);
    } else {
      output.write(`      ${line}\n`);
    }
  });
  output.write("\x1B[2K\x1B[G");
  output.write(`  ${footerMessage || CHECKLIST_FOOTER_DEFAULT}\n`);
}

function finishRawModeChecklist(input, output, resume, onKeypress) {
  output.write("\x1B[?25h");
  input.setRawMode(false);
  input.removeListener("keypress", onKeypress);
  resume();
...
export function selectManyWithRawMode({
  question,
  items,
  ...
  return new Promise((resolve, reject) => {
    ...
    if (initiallySelected) {
      for (const entry of initiallySelected) {
        if (items.some((item) => item.key === entry)) selected.add(entry);
      }
    }

    let focusIndex = 0;
    let footerMessage = "";
    const frameLines = items.length + 2; // question + items + footer

    const rerender = (firstRender) => {
      if (!firstRender) {
        output.write(`\x1B[${frameLines}F`);
      }
      renderChecklistFrame(output, question, items, selected, focusIndex, footerMessage);
    };

    const onKeypress = (str, key = {}) => {
      if (key.ctrl && key.name === "c") {
        finishRawModeChecklist(input, output, resume, onKeypress);
        exitFn(1);
        reject(new Error("Interactive selection aborted by SIGINT."));
        return;
      }
      ...
      if (key.name === "space" || str === " ") {
        ...
        if (selected.has(focused.key)) selected.delete(focused.key);
        else selected.add(focused.key);
        footerMessage = "";
        rerender(false);
        return;
      }
      ...
      if (key.name === "return" || key.name === "enter") {
        if (selected.size === 0 && !allowEmpty) {
          footerMessage = "Select at least one agent. Use Space to toggle, or press 'a' for all.";
          rerender(false);
          return;
        }
        finishRawModeChecklist(input, output, resume, onKeypress);
        ...
        const suffix = count === 1 ? "agent" : "agents";
        output.write(`  \x1B[32m✔\x1B[0m ${question} · \x1B[36m${count} ${suffix} selected\x1B[0m\n`);
...
function buildAgentChecklistItems() {
  return AGENT_DISPLAY_CATALOG.map((agent) => ({
    key: agent.id,
    label: agent.name,
    sublabel: agent.id,
    detail: agent.files ?? "",
...
const LINE_FALLBACK_MAX_ATTEMPTS = 2;
const EMPTY_AGENT_SELECTION_ERROR =
  "No agents selected. Re-run `init` and pick at least one agent, or use `--agents all`.";

async function pickAgentsLineByLineAttempt(prompt, preSelectedIds, headerMessage) {
  console.log("");
  console.log(headerMessage);
  const selected = new Set();
  for (const agent of AGENT_DISPLAY_CATALOG) {
    const detail = agent.files ? ` — ${agent.files}` : "";
    const defaultYes = preSelectedIds.has(agent.id);
    // eslint-disable-next-line no-await-in-loop
    const keep = await prompt.confirm(
      `Include ${agent.name} (${agent.id})${detail}?`,
      defaultYes,
    );
    if (keep) selected.add(agent.id);
  }
  return selected;
}

async function pickAgentsLineByLine(prompt, preSelectedIds) {
  const firstHeader = `  Select each of the ${AGENT_DISPLAY_CATALOG.length} supported agents (y/n):`;
  const retryHeader = `  No agent was selected. Pick at least one — re-asking every agent:`;

  for (let attempt = 0; attempt < LINE_FALLBACK_MAX_ATTEMPTS; attempt += 1) {
    const header = attempt === 0 ? firstHeader : retryHeader;
    // eslint-disable-next-line no-await-in-loop
    const selected = await pickAgentsLineByLineAttempt(prompt, preSelectedIds, header);
    if (selected.size > 0) return selected;
    const inputExhausted = typeof prompt.isInputExhausted === "function"
      && prompt.isInputExhausted();
    if (inputExhausted) break;
  }

  throw new Error(EMPTY_AGENT_SELECTION_ERROR);
}

export async function pickAgentsInteractively(prompt, preSelectedIds) {
  if (typeof prompt.selectAgents === "function") {
    const selected = await prompt.selectAgents(preSelectedIds);
    return new Set([...selected].filter(isKnownAgentId).map(normalizeAgentId));
  }
  const selected = await pickAgentsLineByLine(prompt, preSelectedIds);
  return new Set([...selected].filter(isKnownAgentId).map(normalizeAgentId));
}
...
async function createPrompt() {
  ...
  if (!isTTY) {
    ...
    async function selectAgents(preSelectedIds) {
      return pickAgentsLineByLine({ confirm }, preSelectedIds ?? new Set());
    }

    return { ask, confirm, choose, close, isTTY: false, isInputExhausted, selectAgents };
  }
  ...
  function choose(question, choices, defaultChoice, labels = {}) {
    ...
    if (!supportsRawModeChoice(process.stdin, process.env)) {
      const hint = formatChoiceHint(choiceDefinitions, defaultChoice);
      return new Promise((resolve) => {
        rl.question(`  ${question} ${hint} `, (answer) => {
  ...
  async function selectAgents(preSelectedIds) {
    ...
    const items = buildAgentChecklistItems();
    return selectManyWithRawMode({
      question: "Select agents to support",
      items,
      initiallySelected: preSelectedIds,
      allowEmpty: false,
      input: process.stdin,
      output: process.stderr,
      pause: () => rl.pause(),
  ...
  return { ask, confirm, choose, close, isTTY: true, isInputExhausted, selectAgents };
}
...
function printGreenfieldSelectionHint() {
  ...
  console.log("    - category number: 1-6");
  console.log('    - preset slug: e.g. "fastapi-api"');
  console.log('    - stack name: e.g. "python"');
  console.log('    - generic draft: "skip"');
  console.log('    - stop without writing files: "abort"');
  console.log('  Recommended for a generic or script-based repo: type "skip".');
}

function printOtherStarterGuidance() {
  console.log('  "Other" is for repos that do not fit a starter preset yet.');
  console.log('  Type a stack name like "python", choose a preset slug, type "skip" for a generic draft, or "abort" to stop.');
}

async function promptGreenfieldFollowupSelection(prompt, promptLabel, invalidMessage = null) {
  const subRaw = (await prompt.ask(promptLabel, "")).trim();
  if (!subRaw) {
    console.log('  Type a preset slug, a stack name like "python", "skip", or "abort".');
  ...
  if (!ok) {
    console.log(invalidMessage ? invalidMessage(unknown) : `  Unknown: ${unknown.join(", ")}.`);
    if (prompt.isInputExhausted()) {
      throwStackSelectionIncomplete();
    }
    return null;
...
export async function askGreenfieldStack(prompt) {
  ...
  while (true) {
    ...
    if (!raw) {
      console.log('  No starter selected yet. Choose a category, type a preset slug, type a stack name like "python", or type "skip" for a generic draft.');
      if (prompt.isInputExhausted()) {
        throwStackSelectionIncomplete();
      }
      continue;
    ...
    if (!Number.isNaN(numeric) && String(numeric) === raw.trim()) {
      ...
      if (category.presets === null) {
        ...
        printOtherStarterGuidance();
        const otherTokens = await promptGreenfieldFollowupSelection(
          prompt,
          "Preset slug, stack name, or skip",
          (unknown) => `  Unknown: ${unknown.join(", ")}. Try a preset slug or a stack name like "python".`,
        );
        if (otherTokens === null) continue;
        if (Array.isArray(otherTokens) && otherTokens.length === 0) {
          return buildGreenfieldContext([]);
        }
        return buildGreenfieldContext(otherTokens);
      }
      printPresetList(category.presets, `${category.label} presets`);
      const subTokens = await promptGreenfieldFollowupSelection(
        prompt,
        "Preset slug or stack name",
        (unknown) => `  Unknown: ${unknown.join(", ")}. Try one of: ${category.presets.join(", ")} or a stack name like "python".`,
      );
      if (subTokens === null) continue;
    ...
    console.log('  Type a raw stack name like "python", a preset slug, "skip" for a generic draft, or "abort" to stop.');
    if (prompt.isInputExhausted()) {
      throwStackSelectionIncomplete();
    }
  }
...
export async function runGuidedSetup(targetRoot, baseSpec, options = {}) {
  ...
  try {
    ...
    // --- Agent rollout ---
    let agentSupportSpec;
    if (options.agents) {
      // --agents flag was provided (non-interactive or explicit)
      agentSupportSpec = buildAgentSupportSpec(options.enabledAgents);
    } else {
      const detectedAgentIds = detectCanonicalAgentIds(targetRoot);
      const detectedNames = [...detectedAgentIds]
        .map((id) => AGENT_REGISTRY.get(id)?.displayName)
        .filter(Boolean);

      console.log("");
      if (detectedNames.length > 0) {
        console.log(`  Detected agents in this repo: ${detectedNames.join(", ")}`);
      }

      const agentChoices = detectedNames.length > 0
        ? [
            { key: "1", label: `All ${AGENT_IDS.length} supported agents` },
            { key: "2", label: `Only detected agents (${detectedNames.join(", ")})` },
            { key: "3", label: "Choose specific agents" },
          ]
        : [
            { key: "1", label: `All ${AGENT_IDS.length} supported agents` },
            { key: "3", label: "Choose specific agents" },
      ...
      const defaultAgentChoice = detectedNames.length > 0 ? "2" : "1";
      const agentChoice = await prompt.choose("Agent rollout", agentChoices, defaultAgentChoice);

      if (agentChoice === "2" && detectedAgentIds.size > 0) {
        agentSupportSpec = buildAgentSupportSpec(detectedAgentIds);
      } else if (agentChoice === "3") {
        const preSelected = new Set(detectedAgentIds);
        const selectedIds = await pickAgentsInteractively(prompt, preSelected);
        agentSupportSpec = buildAgentSupportSpec(selectedIds);
      } else {
        // "1" or default — all agents
        agentSupportSpec = undefined;
      }
    }
    ...
    // Apply agent support selection
    if (agentSupportSpec) {
      spec.agentSupport = agentSupportSpec;
    } else {
      delete spec.agentSupport;
    }
