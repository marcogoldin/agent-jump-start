...
import { AGENT_IDS, AGENT_DISPLAY_CATALOG, AGENT_REGISTRY } from "./agent-targets.mjs";
  ...
  try {
    ...
    if (options.agents) {
      ...
      if (agentChoice === "2" && detectedAgentIds.size > 0) {
        ...
        // Let user pick from all agents
        const allAgentEntries = AGENT_DISPLAY_CATALOG.map((a) => `${a.id}: ${a.name}`);
        const preSelected = new Set(detectedAgentIds);
        const { accepted: selectedEntries } = await reviewSuggestedEntries(
          prompt,
          ...
          allAgentEntries,
          {
            kind: "agent",
            defaultKeep: (item) => {
              const agentId = item.split(":")[0];
              return preSelected.has(agentId);
            },
          },
        );
        ...
        if (selectedIds.size === 0) {
          console.log("  No agents selected — defaulting to all.");
