...
async function createPrompt() {
  ...
  function choose(question, choices, defaultChoice, labels = {}) {
    ...
    // Fallback if NO_COLOR is set or raw mode is unavailable
    if (process.env.NO_COLOR || !process.stdin.setRawMode) {
      const hint = formatChoiceHint(choiceDefinitions, defaultChoice);
      return new Promise((resolve) => {
        rl.question(`  ${question} ${hint} `, (answer) => {
    ...
    return new Promise((resolve) => {
      ...
      let selectedIndex = Math.max(0, choiceDefinitions.findIndex(c => c.key === defaultChoice));
      
      const render = () => {
        process.stderr.write('\x1B[?25l'); // hide cursor
        process.stderr.write('\x1B[2K\x1B[G'); // clear line, cursor to start
        process.stderr.write(`  ? ${question}\n`);
        choiceDefinitions.forEach((c, idx) => {
          process.stderr.write('\x1B[2K\x1B[G');
          if (idx === selectedIndex) {
            process.stderr.write(`    \x1B[36m> ${c.label}\x1B[0m\n`);
          } else {
      ...
      const onKeypress = (str, key) => {
        ...
        process.stderr.write(`\x1B[${choiceDefinitions.length + 1}F`);
        ...
        if (key.name === 'up' || key.name === 'k') {
          ...
          process.stderr.write(`  \x1B[32m✔\x1B[0m ${question} · \x1B[36m${choiceDefinitions[selectedIndex].label}\x1B[0m\n`);
          cleanup();
          resolve(choiceDefinitions[selectedIndex].key);
        } else {
          render();
        }
      };
