...
import { describeChain, describeOwner, resolveErrorOwner } from "./layered-diagnostics.mjs";
import { formatValidAgentIds, isKnownAgentId } from "./agent-targets.mjs";
...
export function validateSkill(skill, sourcePath) {
  ...
  for (const rule of skill.rules) {
    ...
    if (rule.semantic !== undefined) {
      const validSemantics = new Set(["directive", "prohibition", "workflow", "example", "reference"]);
      if (!validSemantics.has(rule.semantic)) {
        throw new Error(
          `Invalid skill "${skill.slug}" in ${sourcePath}: rule "${rule.id}" semantic must be one of: ${[...validSemantics].join(", ")}.`,
        );
      }
    }
    ruleIds.add(rule.id);
  ...
  if (skill.triggers !== undefined) {
    if (!Array.isArray(skill.triggers)) {
      throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: triggers must be an array when provided.`);
    }
    for (const [index, trigger] of skill.triggers.entries()) {
      if (!isNonEmptyString(trigger)) {
        throw new Error(
          `Invalid skill "${skill.slug}" in ${sourcePath}: triggers[${index}] must be a non-empty string.`,
        );
      }
    }
  }
  if (skill.globs !== undefined) {
    if (!Array.isArray(skill.globs)) {
      throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: globs must be an array when provided.`);
    }
    for (const [index, glob] of skill.globs.entries()) {
      if (!isNonEmptyString(glob)) {
        throw new Error(
          `Invalid skill "${skill.slug}" in ${sourcePath}: globs[${index}] must be a non-empty string.`,
        );
      }
    }
  }
  if (skill.alwaysApply !== undefined) {
    if (typeof skill.alwaysApply !== "boolean") {
      throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: alwaysApply must be a boolean when provided.`);
    }
  }
  if (skill.manualOnly !== undefined) {
    if (typeof skill.manualOnly !== "boolean") {
      throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: manualOnly must be a boolean when provided.`);
    }
  }
  if (skill.alwaysApply === true && skill.manualOnly === true) {
    throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: alwaysApply and manualOnly cannot both be true.`);
  }
  if (skill.relatedSkills !== undefined) {
    if (!Array.isArray(skill.relatedSkills)) {
      throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: relatedSkills must be an array when provided.`);
    }
    for (const [index, related] of skill.relatedSkills.entries()) {
      if (!isNonEmptyString(related)) {
        throw new Error(
          `Invalid skill "${skill.slug}" in ${sourcePath}: relatedSkills[${index}] must be a non-empty string.`,
        );
      }
    }
  }
  if (skill.compatibility !== undefined) {
    if (!Array.isArray(skill.compatibility)) {
      throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: compatibility must be an array when provided.`);
    }
    for (const [index, client] of skill.compatibility.entries()) {
      if (!isKnownAgentId(client)) {
        throw new Error(
          `Invalid skill "${skill.slug}" in ${sourcePath}: compatibility[${index}] "${client}" is not a recognized agent client. ` +
          `Valid values: ${formatValidAgentIds()}.`,
        );
      }
    }
  }
  ...
  if (skill.scripts !== undefined) {
    if (!Array.isArray(skill.scripts)) {
      throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: scripts must be an array when provided.`);
    }
    for (const [index, script] of skill.scripts.entries()) {
      if (!isPlainObject(script)) {
        throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: scripts[${index}] must be an object.`);
      }
      if (!isNonEmptyString(script.name)) {
        throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: scripts[${index}].name must be a non-empty string.`);
      }
      if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]+$/.test(script.name)) {
        throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: scripts[${index}].name must be a valid filename (e.g. 'setup.sh').`);
      }
      if (!isNonEmptyString(script.content)) {
        throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: scripts[${index}].content must be a non-empty string.`);
      }
    }
  }
  if (skill.assets !== undefined) {
    if (!Array.isArray(skill.assets)) {
      throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: assets must be an array when provided.`);
    }
    for (const [index, asset] of skill.assets.entries()) {
      if (!isPlainObject(asset)) {
        throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: assets[${index}] must be an object.`);
      }
      if (!isNonEmptyString(asset.name)) {
        throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: assets[${index}].name must be a non-empty string.`);
      }
      if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]+$/.test(asset.name)) {
        throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: assets[${index}].name must be a valid filename (e.g. 'template.json').`);
      }
      if (!isNonEmptyString(asset.content)) {
        throw new Error(`Invalid skill "${skill.slug}" in ${sourcePath}: assets[${index}].content must be a non-empty string.`);
      }
    }
  }
}
...
export function validateSpec(spec, sourcePath, options = {}) {
  const errors = [];
  const push = (path, message) => errors.push({ path, message });

  // schemaVersion
  if (spec.schemaVersion === undefined) {
    push("schemaVersion", "Missing required field: schemaVersion");
  } else if (typeof spec.schemaVersion !== "number" || spec.schemaVersion < 1) {
    push("schemaVersion", `Invalid schemaVersion: expected a positive integer, got ${JSON.stringify(spec.schemaVersion)}`);
  }

  // project
  if (!spec.project) {
    push("project", "Missing required field: project");
  } else {
    if (!spec.project.name || typeof spec.project.name !== "string") {
      push("project.name", "project.name is required and must be a non-empty string");
    }
    if (!spec.project.summary || typeof spec.project.summary !== "string") {
      push("project.summary", "project.summary is required and must be a non-empty string");
    }
    if (spec.project.components !== undefined && !Array.isArray(spec.project.components)) {
      push("project.components", "project.components must be an array if present");
    }
  }

  // workspaceInstructions
  if (!spec.workspaceInstructions) {
    push("workspaceInstructions", "Missing required field: workspaceInstructions");
  } else {
    const ws = spec.workspaceInstructions;
    if (ws.sections !== undefined) {
      if (!Array.isArray(ws.sections)) {
        push("workspaceInstructions.sections", "workspaceInstructions.sections must be an array if present");
      } else {
        for (let i = 0; i < ws.sections.length; i++) {
          const section = ws.sections[i];
          if (!section.title || typeof section.title !== "string") {
            push(`workspaceInstructions.sections[${i}].title`, `workspaceInstructions.sections[${i}].title is required and must be a string`);
          }
          if (!Array.isArray(section.rules) || section.rules.length === 0) {
            push(`workspaceInstructions.sections[${i}].rules`, `workspaceInstructions.sections[${i}].rules must be a non-empty array`);
          }
        }
      }
    }
    if (ws.validation !== undefined && !Array.isArray(ws.validation)) {
      push("workspaceInstructions.validation", "workspaceInstructions.validation must be an array if present");
    }
  }
  ...
  if (spec.reviewChecklist !== undefined) {
    ...
    if (!rc.intro || typeof rc.intro !== "string") {
      push("reviewChecklist.intro", "reviewChecklist.intro is required and must be a string");
    }
    if (typeof rc.failureThreshold !== "number" || rc.failureThreshold < 1) {
      push("reviewChecklist.failureThreshold", "reviewChecklist.failureThreshold must be a positive number");
    }
    if (!Array.isArray(rc.items) || rc.items.length === 0) {
      push("reviewChecklist.items", "reviewChecklist.items must be a non-empty array");
    } else {
      for (let i = 0; i < rc.items.length; i++) {
        if (!rc.items[i].title || typeof rc.items[i].title !== "string") {
          push(`reviewChecklist.items[${i}].title`, `reviewChecklist.items[${i}].title is required and must be a string`);
        }
      }
    }
  }
  ...
  if (spec.skills !== undefined) {
    if (!Array.isArray(spec.skills)) {
      push("skills", "skills must be an array if present");
    } else {
      const slugs = new Set();
      for (let i = 0; i < spec.skills.length; i++) {
        ...
        try {
          ...
          push(`skills[${i}]`, err.message);
        }
        if (skill.slug) {
          if (slugs.has(skill.slug)) {
            push(`skills[${i}]`, `Duplicate skill slug: "${skill.slug}" at skills[${i}]`);
          }
  ...
  if (errors.length > 0) {
    throw new Error(formatValidationErrors(errors, sourcePath, options));
  }
}

/**
 * Format collected validation errors into a single operator-facing message.
 *
 * When a layered spec context is provided (`options.layerChain` plus
 * `options.leafPath`), each error gets annotated with the layer that owns
 * the offending top-level field, and the header names the full extends
 * chain so the operator can see which files are merged into this result.
 */
function formatValidationErrors(errors, sourcePath, options) {
  const chain = Array.isArray(options?.layerChain) ? options.layerChain : null;
  const isLayered = Boolean(chain && chain.length > 1);
  const leafPath = options?.leafPath ?? null;

  const countLabel = `${errors.length} error${errors.length > 1 ? "s" : ""}`;
  const headerLines = [`Spec validation failed for ${sourcePath} (${countLabel}):`];
  if (isLayered) {
    headerLines.push(`  Layer chain: ${describeChain(chain)}`);
  }

  const bodyLines = errors.map((entry, i) => {
    const base = `  ${i + 1}. ${entry.message}`;
    if (!isLayered) return base;
    const ownerPath = resolveErrorOwner(chain, entry.path, leafPath);
    const ownerHint = describeOwner(ownerPath);
    return ownerHint ? `${base}\n     ${ownerHint}` : base;
  });
...
function describeUnexpectedValue(value) {
  if (value === undefined) return "Received: undefined (field is missing).";
  if (value === null) return "Received: null.";
  if (Array.isArray(value)) return `Received: array with ${value.length} item(s).`;
  if (typeof value === "object") {
    const keys = Object.keys(value);
    if (keys.length === 0) {
      return "Received: empty object {}. Hint: this often means a YAML block scalar (e.g. `description: >`) could not be parsed — ensure the block is correctly indented.";
    }
    return `Received: object with keys [${keys.slice(0, 5).join(", ")}].`;
  }
  if (typeof value === "string") return "Received: empty or whitespace-only string.";
  return `Received: ${typeof value} (${String(value)}).`;
}
...
export function validateSkillMdFrontmatter(frontmatter, sourcePath) {
  ...
  if (!isNonEmptyString(frontmatter.name)) {
    errors.push(
      `frontmatter.name is required and must be a non-empty string. ${describeUnexpectedValue(frontmatter.name)}`,
    );
  }
  if (!isNonEmptyString(frontmatter.description)) {
    errors.push(
      `frontmatter.description is required and must be a non-empty string. ${describeUnexpectedValue(frontmatter.description)}`,
    );
  }
