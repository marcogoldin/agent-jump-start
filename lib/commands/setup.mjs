...
export async function handleInit(options) {
  ...
  if (options.guided) {
    ...
    mergedSpec = await runGuidedSetup(targetRoot, mergedSpec, { profileApplied: Boolean(options.profile) });
  } else {
  ...
  console.log(`\nDone! Next steps:`);
  if (options.guided) {
    console.log(`  1. Review ${specRel} and refine the starter draft until it matches the real repo`);
    console.log(`  2. The files rendered above are starter outputs from your confirmed answers`);
    console.log(`  3. Re-run: node ${scriptRel} sync --spec ${specRel} after refining the spec`);
  } else {
  ...
  if (options.guided) {
    console.log(`  4. Commit both the spec and the generated files once they reflect the real repo`);
  }
}
