...
import { readManifestPreservedPaths } from "../files.mjs";
...
function filterAbsorbableSources(discovered, targetRoot, preservedOnly) {
  const absorbable = discovered.filter((entry) =>
    entry.classification === "unmanaged" || entry.classification === "unreadable");
  if (!preservedOnly) return absorbable;

  const preservedPaths = readManifestPreservedPaths(targetRoot);
  if (preservedPaths.size === 0) return [];
  return absorbable.filter((entry) => preservedPaths.has(entry.relativePath));
}
...
export async function handleAbsorb(options) {
  ...
  const preservedOnly = Boolean(options["preserved-only"]);
  ...
  const absorbable = filterAbsorbableSources(discovered, targetRoot, preservedOnly);
  const candidates = extractAbsorbCandidates(absorbable);

  if (candidates.length === 0) {
    if (preservedOnly) {
      console.log("Nothing to absorb: no preserved operator-authored agent instruction files were found.");
    } else {
