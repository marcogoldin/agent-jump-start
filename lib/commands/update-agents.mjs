...
import { AGENT_REGISTRY, formatValidAgentIds, isKnownAgentId, normalizeAgentId } from "../agent-targets.mjs";
...
export async function handleUpdateAgents(options) {
  ...
  if (options.mode === "all" || options["all-missing"]) {
    ...
    printSyncHint(options.spec, { purgeDisabledRecommended: true });
    return;
  }

  if (options.include) {
    const rawIdsToAdd = options.include.split(",").map((s) => s.trim()).filter(Boolean);
    const invalid = rawIdsToAdd.filter((id) => !isKnownAgentId(id));
    if (invalid.length > 0) {
      throw new Error(`Unknown agent ID(s): ${invalid.join(", ")}. Valid IDs: ${formatValidAgentIds()}`);
    }
    const idsToAdd = [...new Set(rawIdsToAdd.map(normalizeAgentId))];
  ...
  if (options.remove) {
    const rawIdsToRemove = options.remove.split(",").map((s) => s.trim()).filter(Boolean);
    const invalid = rawIdsToRemove.filter((id) => !isKnownAgentId(id));
    if (invalid.length > 0) {
      throw new Error(
        `Unknown agent ID(s): ${invalid.join(", ")}. ` +
        `Valid IDs: ${formatValidAgentIds()}. ` +
        `Run 'agent-jump-start list-agents' to see all available agents.`,
      );
    }
    const idsToRemove = [...new Set(rawIdsToRemove.map(normalizeAgentId))];
  ...
  printSyncHint(options.spec, { purgeDisabledRecommended: result.removed.length > 0 });
}
...
function printSyncHint(specPath, { purgeDisabledRecommended = false } = {}) {
  console.log(`\nRun 'agent-jump-start sync --spec ${specPath}' to regenerate files.`);
  if (purgeDisabledRecommended) {
    console.log(`Run 'agent-jump-start sync --spec ${specPath} --purge-disabled' to fully remove files for disabled agents, including residual agent-only directories.`);
  }
}
...
function resolveInteractiveSelection(input, candidateIds) {
  ...
  for (const token of tokens) {
    ...
    } else if (isKnownAgentId(token)) {
      selected.push(normalizeAgentId(token));
    }
  }
