...
import { discoverUnmanagedSkills, inspectSkillSource } from "../intake.mjs";
...
export function importSkillsIntoSpec(specPathInput, sourcePathInput, replaceExisting = false, sourceInfo = null, importOptions = {}) {
  ...
  // P0-0 guard: refuse to parse Agent Jump Start generated SKILL.md through
  // readExternalSkill. The JSON path and standalone SKILL.md path funnel
  // through inspectSkillSource so both flows see the rejection consistently
  // with validate-skill.
  assertNotGeneratedArtifact(sourcePathInput);
 ...
 *
 * Two-stage validation that matches the canonical import pipeline, so a
 * SKILL.md that passes this command will also pass `import-skill` (and a
 * directory-backed skill will also pass `intake --import`):
 *
 ...
 *
 * Possible outcomes:
 *   - `import-compatible` → exit 0. Canonical import will accept this skill.
 ...
 *
 * Standalone `.md` files also receive canonical reconstruction by default; the
 * sibling `references/`, `scripts/`, and `assets/` folders are discovered from
 * the file's containing directory exactly as `import-skill` does.
 *
...
export function handleValidateSkill(options, args) {
  ...
  if (existsSync(join(absolutePath, "SKILL.md"))) {
    ...
    const inspected = inspectSkillSource(absolutePath);
    reportInspectedSkill(inspected, skillPath);
    return;
  }

  // Standalone SKILL.md file — defaults to the same canonical reconstruction
  // path that import-skill uses.
  if (absolutePath.endsWith(".md") && existsSync(absolutePath)) {
    if (frontmatterOnly) {
      validateStandaloneSkillMdFrontmatterOnly(absolutePath, skillPath);
      return;
    }
...
function validateStandaloneSkillMdFrontmatterOnly(absolutePath, skillPath) {
  ...
  console.log(`  description: ${frontmatter.description.slice(0, 80)}...`);
  console.log("  note: --frontmatter-only skipped canonical import-compatibility check.");
  console.log("        Drop the flag to get the same verdict canonical import will give you.");
}
...
function reportInspectedSkill(inspected, displayedPath) {
  if (inspected.status === "import-compatible") {
    ...
    const refsBaseDir = inspected.path.endsWith(".md") ? dirname(inspected.path) : inspected.path;
    const refsDir = join(refsBaseDir, "references");
    if (existsSync(refsDir)) {
      const refFiles = readdirSync(refsDir).filter((f) => f.endsWith(".md"));
      console.log(`  references: ${refFiles.length} file(s)`);
    }
    console.log("  canonical import will accept this skill.");
    return;
  }

  if (inspected.status === "generated-artifact") {
    const lines = [
      `validate-skill: rejected (Agent Jump Start generated artifact) — ${displayedPath}`,
      ...inspected.errors.map((e) => `  - ${e}`),
      "",
      "Canonical import would corrupt the spec with synthetic `rcbp-*` / `qr-*` / `dg-*` ids.",
    ];
  ...
  if (inspected.status === "structurally-valid") {
    const lines = [
      ...
      ...inspected.errors.map((e) => `  - ${e}`),
      "",
      "Canonical import would reject this skill with the same errors.",
...
/**
 * Refuse to import a source path that points to an Agent Jump Start
 * generated SKILL.md (directory or standalone file). Bare JSON sources
 * and non-existent paths fall through so downstream readers can produce
 * their usual errors.
 */
function assertNotGeneratedArtifact(sourcePathInput) {
  const absolute = resolve(sourcePathInput);
  const isDir = existsSync(join(absolute, "SKILL.md"));
  const isStandaloneMd = absolute.endsWith(".md") && existsSync(absolute);
  if (!isDir && !isStandaloneMd) return;

  const inspected = inspectSkillSource(absolute);
  if (inspected.status !== "generated-artifact") return;

  const lines = [
    `Refusing to import ${sourcePathInput}.`,
    ...inspected.errors,
    "",
    "Canonical import would corrupt the spec with synthetic `rcbp-*` / `qr-*` / `dg-*` ids.",
  ];
