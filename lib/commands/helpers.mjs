...
/**
 * Resolve a spec path, follow any `extends` chain, validate the merged
 * result with layer-aware diagnostics, and return the merged spec plus
 * layer metadata.
 *
 * Callers that need the raw leaf spec (e.g. for leaf-only writeback) can
 * access it via the returned `leafSpec` / `leafPath` / `isLayered` fields.
 */
export function resolveAndValidateSpec(specPath) {
  const meta = resolveLayeredSpecWithMeta(specPath);
  validateSpec(meta.merged, specPath, {
