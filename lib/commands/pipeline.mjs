...
function cleanDisabledTargetsForCommand(enabledAgents, targetRoot, options) {
  return cleanDisabledAgentRoots(enabledAgents, targetRoot, AGENT_REGISTRY, {
    purge: Boolean(options["purge-disabled"]),
...
export async function handleSync(options) {
  ...
  const rootCleanup = cleanDisabledTargetsForCommand(enabledAgents, targetRoot, options);
  allBackedUp = [...allBackedUp, ...(rootCleanup.backedUp ?? [])];
  if (rootCleanup.removed.length > 0) {
    const header = options["purge-disabled"]
      ? "Purged disabled-agent targets:"
      : "Cleaned disabled-agent targets:";
    console.log(header);
  ...
  if (rootCleanup.residuals.length > 0) {
    const header = options["purge-disabled"]
      ? "Disabled-agent targets still left in place:"
      : "Disabled-agent directories with unmanaged content (left in place):";
    console.log(header);
    for (const r of rootCleanup.residuals) {
      const nextStep = options["purge-disabled"]
        ? "review manually"
        : "review manually or rerun sync --purge-disabled";
      console.log(`  - ${r.root} (${r.displayName}) — ${nextStep}`);
    }
    console.log("");
  }
  ...
  // This is a CLI contract change (v1.19.0): sync --keep-existing exits 2
  // when preserved files prevent full convergence, instead of silently exiting 0.
  // Since v2.0.2, standalone 'check' mirrors this contract and also exits 2
  // on the same preserved state (previously it exited 1 as drift).
  ...
  if (allKept.length > 0) {
    ...
    console.log("The project is NOT fully converged. 'check' will report these files under a Preserved section and also exit 2.\n");
    console.log("To converge, choose one of:");
    console.log(`  promote:    agent-jump-start promote-preserved --spec ${options.spec} --target ${options.target ?? "."}`);
    console.log(`              Keep the intent and patch the canonical spec from preserved files.`);
    console.log(`  overwrite:  agent-jump-start sync --spec ${options.spec} --force`);
    console.log(`              Discard the preserved hand-edit and rewrite files from the current spec.`);
    console.log(`  backup:     agent-jump-start sync --spec ${options.spec} --backup`);
    console.log(`              Discard the hand-edit, but keep timestamped backups before rewrite.`);
    process.exit(2);
  }
  ...
  if (options.clean) {
    ...
    if (rootCleanup.residuals.length > 0) {
      ...
      for (const r of rootCleanup.residuals) {
        const nextStep = options["purge-disabled"]
          ? "review manually"
          : "review manually or rerun render --clean --purge-disabled";
        console.log(`  - ${r.root} (${r.displayName}) — ${nextStep}`);
      }
      console.log("");
    }
 ...
 *
 * Exit code contract (aligned with sync v1.19.0+):
 *   0 — fully converged; no drift, no preserved paths
 *   1 — genuine drift detected (missing, content mismatch, stale managed)
 *   2 — safe but non-converged; only operator-preserved paths differ from spec
 *
 * @param {object} options — parsed CLI options
 */
export function handleCheck(options) {
  ...
  const generatedFiles = renderGeneratedFiles(spec, options.spec, targetRoot, { enabledAgents });
  const { failures, passes, preserved = [] } = checkGeneratedFiles(generatedFiles, targetRoot);
  ...
  if (failures.length > 0) {
    printFailureReport(failures);
    // Hint about preserved-like files among genuine drift failures so
    // operators who edited files after preservation still get guidance.
    ...
    if (possiblyPreserved.length > 0) {
      ...
      console.log(`To converge: run 'promote-preserved --spec ${options.spec} --target ${options.target ?? "."}' to keep the intent and patch the spec, or 'sync --spec ${options.spec} --force' to discard the hand-edit and rewrite from spec.`);
    }
    if (preserved.length > 0) {
      printPreservedSection(preserved);
    }
  ...
  if (preserved.length > 0) {
    printPreservedSection(preserved);
    console.log(`\nCheck is safe but non-converged: ${preserved.length} file(s) preserved by a prior 'sync --keep-existing'.`);
    console.log("No unexpected drift detected. To converge, choose one of:");
    console.log(`  promote:    agent-jump-start promote-preserved --spec ${options.spec} --target ${options.target ?? "."}`);
    console.log(`              Keep the intent and patch the canonical spec from preserved files.`);
    console.log(`  overwrite:  agent-jump-start sync --spec ${options.spec} --force`);
    console.log(`              Discard the preserved hand-edit and rewrite files from the current spec.`);
...
function printPreservedSection(preserved) {
  console.log("\nPreserved (operator-authored, intentionally kept by a prior sync --keep-existing):");
  for (const entry of preserved) {
    const stamp = entry.preservedAt ? ` — preserved at ${entry.preservedAt}` : "";
    console.log(`  - ${entry.relativePath}${stamp}`);
  }
}
