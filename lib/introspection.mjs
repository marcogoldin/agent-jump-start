...
function collectMatchingFiles(targetRoot, relativeDir, { extensions = [], filenames = [], ignoreDirs = [], suffixes = [], maxFiles = 40 } = {}) {
  ...
  const matches = [];
  const stack = [baseDir];
  const ignoredDirNames = new Set(ignoreDirs);
  ...
  while (stack.length > 0 && matches.length < maxFiles) {
    ...
    for (const entry of entries) {
      ...
      const matchesFilename = filenames.length === 0 || filenames.some((filename) => lowerName === filename.toLowerCase());
      ...
      if (!matchesFilename || !matchesExtension || !matchesSuffix) continue;
...
export function introspectProject(targetRoot) {
  ...
  for (const rule of AGENT_DISCOVERY_RULES) {
    if (rule.mode === "exact") {
      if (!existsSync(join(targetRoot, rule.path))) continue;
      result.signals.push({
        type: "agent",
        file: rule.path,
        detail: `Existing agent instructions (${rule.path})`,
      });
      continue;
    }

    for (const relPath of collectMatchingFiles(targetRoot, rule.root, {
      extensions: rule.extensions,
      filenames: rule.filenames,
      ignoreDirs: rule.ignoreDirs,
      suffixes: rule.suffixes,
    })) {
      result.signals.push({
        type: "agent",
        file: relPath,
        detail: `Existing agent instructions (${relPath})`,
      });
    }
  }
