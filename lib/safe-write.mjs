...
import { applyWriteDecisions, classifyPreWriteCollisions, mergeManifestPreservedState } from "./files.mjs";
...
function classifyWriteImpact(decisions) {
  const kept = [];
  const overwritten = [];
  for (const [path, decision] of Object.entries(decisions)) {
    if (decision === "keep") kept.push(path);
    else if (decision === "overwrite" || decision === "backup-then-overwrite") overwritten.push(path);
  }
  return { kept, overwritten };
}

function applyPreservedStateToManifest(generatedFiles, targetRoot, { newlyKept, newlyOverwritten, stamp } = {}) {
  if (!(MANIFEST_PATH in generatedFiles)) return generatedFiles;
  const nextManifestContent = mergeManifestPreservedState(
    generatedFiles[MANIFEST_PATH],
    targetRoot,
    { newlyKept, newlyOverwritten, stamp },
  );
  if (nextManifestContent === generatedFiles[MANIFEST_PATH]) return generatedFiles;
  return { ...generatedFiles, [MANIFEST_PATH]: nextManifestContent };
}
...
export async function safeWriteGeneratedFiles({
  ...
  if (unmanaged.length === 0) {
    // No collisions in this run, but any previously preserved state still
    // needs to be carried over so the freshly rendered manifest does not
    // wipe preserved[] from disk.
    const effectiveFiles = applyPreservedStateToManifest(generatedFiles, targetRoot, {
      newlyKept: [],
      newlyOverwritten: [],
    });
    const res = applyWriteDecisions(effectiveFiles, targetRoot);
    return { ...res, blocked: [], effectiveFiles };
  }

  const policyDecisions = buildDecisionMap(unmanaged, conflictPolicy);
  if (policyDecisions) {
    const { kept: newlyKept, overwritten: newlyOverwritten } = classifyWriteImpact(policyDecisions);
    const effectiveFiles = applyPreservedStateToManifest(generatedFiles, targetRoot, {
      newlyKept,
      newlyOverwritten,
    });
  ...
  const { kept: newlyKept, overwritten: newlyOverwritten } = classifyWriteImpact(decisions);
  const effectiveFiles = applyPreservedStateToManifest(generatedFiles, targetRoot, {
    newlyKept,
    newlyOverwritten,
  });
