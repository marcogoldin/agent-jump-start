...
export function mergeByKey(baseArray, overlayArray, keyField) {
  ...
  for (const item of overlayArray) {
    if (item[keyField] !== undefined) {
      overlayMap.set(item[keyField], item);
    }
  }
...
export function resolveLayeredSpec(specPath) {
  ...
  // Raccoglie i layer dalla foglia alla radice
  while (true) {
  ...
  // La catena è [foglia, ..., radice].  Ribaltiamo per applicare
  // i merge da radice → intermediari → foglia.
  chain.reverse();
