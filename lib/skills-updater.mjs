...
export function refreshSkills({ specPath, lockfilePath, dryRun = false, slugFilter = null }) {
  ...
  // Leggiamo la spec corrente (per riscriverla se necessario)
  const spec = readJsonYaml(absoluteSpecPath);
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // Applicazione: sostituisce la skill nella spec
      const existingIndex = spec.skills.findIndex((s) => s.slug === entry.slug);
      if (existingIndex >= 0) {
        spec.skills[existingIndex] = freshSkill;
      } else {
        spec.skills.push(freshSkill);
      }
      specDirty = true;
  ...
  if (!dryRun) {
    if (specDirty) {
      writeFileSync(absoluteSpecPath, stringifyJsonYaml(spec), "utf8");
    }
