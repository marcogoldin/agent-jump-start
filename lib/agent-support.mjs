...
export function resolveEnabledAgents(spec) {
  ...
  return new Set(support.selected.map(normalizeAgentId));
}
...
export function resolveAgentsForInit(cliAgentsFlag, targetRoot) {
  ...
  // Comma-separated list of canonical IDs. Deprecated aliases are normalized
  // immediately so downstream render/check logic has one internal shape.
  const ids = cliAgentsFlag.split(",").map((s) => s.trim()).filter(Boolean);
  const invalid = ids.filter((id) => !isKnownAgentId(id));
  if (invalid.length > 0) {
  ...
  return new Set(ids.map(normalizeAgentId));
}

function hasRuleMatch(fileName, rule) {
  if (Array.isArray(rule.filenames) && rule.filenames.includes(fileName)) {
    return true;
  }
...
function treeContainsMatchingFile(rootDir, rule) {
  ...
  const ignoredDirNames = new Set(rule.ignoreDirs ?? []);
  ...
  while (pending.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if ((entry.isFile() || entry.isSymbolicLink()) && hasRuleMatch(entry.name, rule)) {
        return true;
      }
    }
  }
...
export function buildAgentSupportSpec(enabledAgents) {
  ...
  const canonicalEnabledAgents = new Set([...enabledAgents].map(normalizeAgentId));
  if (canonicalEnabledAgents.size >= AGENT_IDS.length) return undefined;

  return {
    mode: "selected",
    selected: AGENT_IDS.filter((id) => canonicalEnabledAgents.has(id)),
  };
}
...
function agentOwnsGeneratedPath(agentId, relativePath, targetRoot) {
  ...
  const registryEntry = AGENT_REGISTRY.get(normalizedAgentId);
  if (!registryEntry) return false;

  const renderPaths = [...registryEntry.renderPaths];
  if (normalizedAgentId === "cline") {
